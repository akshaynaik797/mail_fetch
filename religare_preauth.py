...
try:		
	hg=[]
	if f.find('Claim Number AL:')!=-1:
                w=f.find('Claim Number AL:')+16
                g=f[w:]
                u=g.find('(')+w
                hg.append(f[w:u])
	
	elif f.find('Claim Intimation No:')!=-1:
                w=f.find('Claim Intimation No:')+20
                g=f[w:]
                u=g.find('(')+w
                hg.append(f[w:u])
	w=f.find('Total Approved amount')+21
	g=f[w:]
	u=g.find('(')+w
	hg.append(f[w:u])
	status='Approved'
	w=f.find('Policy No')+13
	g=f[w:]
	u=g.find('Expected')+w
	hg.append(f[w:u])

	w=f.find('Patient’s Member')+17
	g=f[w:]
	u=g.find('\n')+w
	hg.append(f[w:u])
	...
	try:
		regex_list = [r"(?<=Patient Name).*(?=Age)", r"(?<=submitted by).*(?=we)"]
