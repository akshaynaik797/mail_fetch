...
try:
    ...
    if f.find('Requested Amount:') != -1:
        x = re.search(r"(?<=Requested Amount:).*", f)
        x1 = x.group()
        hg.append(x1.strip())
    else:
        hg.append('')

    if f.find('Patient CardId:') != -1:
        x = re.search(r"(?<=Patient CardId:).*", f)
        x1 = x.group()
        hg.append(x1.strip())
    else:
        hg.append('')
    ...
    try:
        subprocess.run(
            ["python", "test_api.py", hg[0], hg[3], hg[2], '', 'Acknowledgement', sys.argv[6], sys.argv[1], '', hg[4], hg[1]])
        '''wbk= openpyxl.load_workbook(wbkName)
        s2=wbk.worksheets[1]
