...
try:
	...
	status='Approved'
	w=f.find('payment of Rs')+13
	g=f[w:]
	u=g.find('(')+w
	hg.append(f[w:u])
	
	w=f.find('UHID Number')+12
	g=f[w:]
	u=g.find(':')+w+1
	g1=f[u:]
	u1=g1.find(' ')+u
	...
	try:
		regex_list = [r"(?<=Name of the Patient).*(?=Policy)"]
		pname = pname_fun(f, regex_list)
		subprocess.run(["python", "test_api.py",hg[0],hg[1],hg[3],'',status,sys.argv[6],sys.argv[1],'',hg[2], pname])
		'''wbk= openpyxl.load_workbook(wbkName)
		s2=wbk.worksheets[1]
