...
try:
    ...
    regex = r"(?<=File Number:).*"
    if f.find("File No"):
        temp = re.compile(r"(?<=File No).*").search(f)
        if temp is not None:
            hg.append(temp.group().strip())
    elif f.find("File Number"):
    ...
    regex = r"(?<=Policy Number:).*(?=Held)"
    if f.find('Policy No :') != -1:
        w = f.find('Policy No :') + 60
        g = f[w:]
        u = g.find('\n') + w
        hg.append(f[w:u])
    elif f.find("Policy Number:"):
        temp = re.compile(regex).search(f)
        if temp is not None:
            hg.append(temp.group().strip())
    else:
    ...
    try:
        regex_list = [r"(?<=Cashless Facility for).*"]
