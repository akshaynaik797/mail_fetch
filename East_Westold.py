...
def read_email_from_gmail():
    ...
    for i in range(0, len(id_list)):
       ...
       try:
                ...
                log_exceptions(data=data[0][1], eid=latest_email_id, typed=type(data[0][1]))
        ...
        if email_message['Subject'] not in fg:
            for part in email_message.walk():
                ...
                detach_dir = (os.getcwd() + '/East_West/attachments_' + str(sys.argv[6]))
                if bool(fileName):
...
if path.exists(r'East_West/East_West' + str(sys.argv[6]) + '.xlsx'):
    os.remove(r'East_West/East_West' + str(sys.argv[6]) + '.xlsx')
import openpyxl

po = []
wbkName = 'East_West/East_West' + str(sys.argv[6]) + '.xlsx'
...
for t in range(0, len(onlyfiles)):
    try:
        ...
        tables.export('East_West/foo1.xls', f='excel')
        loc = ("East_West/foo1.xls")
        ...
        os.rename(os.getcwd() + '/East_West/attachments_' + str(sys.argv[6]) + '/' + onlyfiles[t],
                  os.getcwd() + '/East_West/attachments_' + str(sys.argv[6]) + '/' + hg[0] + '.pdf')
