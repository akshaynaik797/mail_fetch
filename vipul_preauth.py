...
try:
    ...
    if f.find('Claim Number') != -1:
        ...
        temp = re.compile(r"(?<=Claim).*").search(f)
        if temp is not None:
    ...
    status = 'Approved'
    if f.find('Total Authorized amount') != -1:
        w = f.find('Total Authorized amount') + 23
        g = f[w:]
        u = g.find('(') + w
        hg.append(f[w:u])
    elif f.find('Total Authorized'):
        temp = re.compile(r"(?<=Total Authorized)[ :\d]+").search(f)
        if temp is not None:
            hg.append(temp.group().strip())
        else:
            hg.append("")
    if f.find('Policy Number') != -1:
        ...
        u = g.find('Expected') + w
        hg.append(f[w:u])
    elif f.find('Policy') != -1:
        temp = re.compile(r"(?<=Policy).*(?=Expected)").search(f)
        if temp is not None:
            hg.append(temp.group().strip())
        else:
            hg.append("")
    if f.find('Member ID') != -1:
