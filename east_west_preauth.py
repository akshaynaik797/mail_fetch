...
try:
	...
	w=f.find('Authorized amount')+16
	g=f[w:]
	u=g.find('/')+w
	hg.append(f[w:u])

	temp = re.compile(r"(?<=Policy No.) +\w+").search(f)
	if temp is not None:
		polno = temp.group().strip()
	else:
		polno = ""
	hg.append(polno)
	...
	hg=[sub.replace('t','') for sub in hg]
	hg=[sub.replace(':','') for sub in hg]
	hg=[sub.replace('  ','') for sub in hg]
	hg=[sub.replace('\n','') for sub in hg]
	hg=[sub.replace('-','') for sub in hg]
	...
	try:
		subprocess.run(["python", "test_api.py",hg[0],hg[1],hg[2],' ','Approved',sys.argv[6],sys.argv[1],' ',hg[3], pname])
		'''wbk= openpyxl.load_workbook(wbkName)
		s2=wbk.worksheets[1]
