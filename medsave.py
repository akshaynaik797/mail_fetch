...
try:
    ...
    repeat = []
    wq = 0
    # sys.argv = ['medsave.py', 'Tpappg@maxhealthcare.com', 'May@2020', '24-Aug-2020', '24-Aug-2020', 'outlook.office365.com', 'Max', '28032']
    ...
    def read_email_from_gmail():
        ...
        if mail_uid == -1:
            type, data = mail.search(None,
                                     '(SUBJECT "Claim Settlement(Cashless) of Patient Name" since ' + srt + ' before ' + stp + ')')
            ids = data[0]
            id_list = ids.split()
        else:
        ...
        for i in range(0, len(id_list)):
            ...
            if email_message['Subject'] not in fg:
                for part in email_message.walk():
                    ...
                    detach_dir = (os.getcwd() + '/medsave/attachments_' + str(sys.argv[6]))
                    if bool(fileName):
    ...
    if path.exists(r'medsave/medsave' + str(sys.argv[6]) + '.xlsx'):
        os.remove(r'medsave/medsave' + str(sys.argv[6]) + '.xlsx')
    import openpyxl

    po = []
    wbkName = 'medsave/medsave' + str(sys.argv[6]) + '.xlsx'
    ...
    for t in range(0, len(onlyfiles)):
        try:
            sh1 = ['Sr No.', 'Claim No', 'Name of Patient', 'Employee ID', 'Employee Name', 'Policy Number', 'Member Id',
            ...
            if tables.n != 0:
                tables.export('medsave/foo1.xls', f='excel')
                loc = ("medsave/foo1.xls")
            ...
            os.rename(os.getcwd() + '/medsave/attachments_' + str(sys.argv[6]) + '/' + onlyfiles[t],
                      os.getcwd() + '/medsave/attachments_' + str(sys.argv[6]) + '/' + hg[0] + '.pdf')

        except Exception as e:
            log_exceptions()
            s1.cell(row=t + 2, column=1).value = 'error'
            s1.cell(row=t + 2, column=2).value = onlyfiles[t]
        # os.rename(os.getcwd()+'/medsave/attachments_'+str(sys.argv[6])+'/'+onlyfiles[t],os.getcwd()+'/medsave/attachments_'+str(sys.argv[6])+'/'+hg[0]+'.pdf')
    ...
    row_ = s1.max_row + 1
    s1.cell(row=row_, column=1).value = 'medsave'
